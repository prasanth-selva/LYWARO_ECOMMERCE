// Blacktop Editorial: navigation is quiet over carbon, then resolves into a warm solid rail after scroll.
      ...
      <header className={`fixed inset-x-0 top-0 z-40 border-b transition-all duration-300 ${scrolled || location !== "/" ? "border-[#111211]/10 bg-[#EAE8E1]/90 text-[#111211] backdrop-blur-xl" : "border-white/10 bg-[#111211]/15 text-[#EAE8E1] backdrop-blur-md"}`}>
        <div className="mx-auto flex h-[74px] max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12">
          <Link href="/" className="group flex items-center gap-3" onClick={() => setMenuOpen(false)}>
            <span className={`grid h-8 w-8 place-items-center border transition ${scrolled || location !== "/" ? "border-[#111211]" : "border-white/50"}`}><img src="/manus-storage/lywaro-apex-mark_aacf2bcf.png" alt="" className="h-5 w-5 object-contain" /></span>
          ...
          <div className="flex items-center gap-1">
            <button className="hidden h-10 w-10 place-items-center opacity-65 transition hover:text-[#D7F54A] hover:opacity-100 sm:grid" aria-label="Search"><Search size={16} strokeWidth={1.5} /></button>
            <button className="hidden h-10 w-10 place-items-center opacity-65 transition hover:text-[#D7F54A] hover:opacity-100 sm:grid" aria-label="Wishlist"><Heart size={16} strokeWidth={1.5} /><span className="sr-only">{wishlist.length} items saved</span></button>
