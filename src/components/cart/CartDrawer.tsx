      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 bottom-0 z-50 w-full max-w-md bg-lywaro-dark border-l border-white/5 flex flex-col"
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  <ShoppingBag size={48} className="text-lywaro-gray/30 mb-4" />
                  <p className="text-sm font-bold tracking-wider text-white/30">YOUR BAG IS EMPTY</p>
                  <p className="text-xs text-lywaro-gray/50 mt-2">Add items to get started</p>
                  <button
                    onClick={closeCart}
                ...
                items.map((item) => (
                  ...
                  >
                    {/* Image */}
                    <div className="w-20 h-20 bg-lywaro-dark flex items-center justify-center flex-shrink-0">
                      <span className="text-lg font-bold text-white/10">{item.product.name[0]}</span>
                    </div>

                    {/* Details */}
                    <div className="flex-1 min-w-0">
                      <h4 className="text-sm font-bold text-white tracking-wider">{item.product.name}</h4>
                      <p className="text-xs text-lywaro-gray mt-0.5">{item.color} / {item.size}</p>
            ...
            {items.length > 0 && (
              <div className="border-t border-white/5 p-6 space-y-4">
                ...
                <Link
                  to="/cart"
                  onClick={closeCart}
                  className="block w-full text-center py-3 border border-white/20 text-xs font-bold tracking-wider text-lywaro-gray hover:border-white/50 hover:text-white transition-colors"
                >
                  VIEW CART
                </Link>
                <Link
                  to="/checkout"
                  onClick={closeCart}
                  className="block w-full text-center py-3 bg-white text-black text-xs font-bold tracking-wider hover:bg-lywaro-crimson hover:text-white transition-colors"
                >
                  CHECKOUT
                </Link>
              </div>
